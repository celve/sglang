...
// Map cuBLAS API to hipBLAS for JIT compilation
#define cublasHgemm __compat_hipblasHgemm
...
void gemm_half_q_half_cuda(
  ...
  if (use_reconstruct) {
    ...
    if (use_shuffle) {
      reconstruct_exllama(b_q_weight, b_gptq_qzeros, b_gptq_scales, b_g_idx, temp_dq, size_k, size_n, groups, bit, stream);
    } else {
      reconstruct_gptq(b_q_weight, b_gptq_qzeros, b_gptq_scales, b_g_idx, temp_dq, size_k, size_n, groups, bit, stream);
    }

    cublasHandle_t cublas_handle = get_cublas_handle(stream);
    ...
    gemm_half_q_half_alt(a, b_q_weight, b_gptq_qzeros, b_gptq_scales, b_g_idx, c, size_m, size_n, size_k, bit, stream);
  }
}
...
void shuffle_exllama_weight(uint32_t* q_weight, int* q_perm, int height, int width, int bit, cudaStream_t stream) {
  if (q_perm) {
    ...
    // Replace qweights
    cudaMemcpyAsync(q_weight, new_qweight, height / 32 * bit * width * sizeof(uint32_t), cudaMemcpyDeviceToDevice, stream);
    // Cleanup
...
void gptq_gemm(
  ...
  int size_n = static_cast<int>(b_q_weight.size(1));
  int groups = static_cast<int>(b_gptq_qzeros.size(0));

  const int* g_idx_ptr = b_g_idx.size(0) == 0 ? nullptr
      : reinterpret_cast<const int*>(b_g_idx.data_ptr());

  sglang::gptq::gemm_half_q_half_cuda(
      stream,
      reinterpret_cast<const half*>(a.data_ptr()),
      reinterpret_cast<const uint32_t*>(b_q_weight.data_ptr()),
      reinterpret_cast<const uint32_t*>(b_gptq_qzeros.data_ptr()),
      reinterpret_cast<const half*>(b_gptq_scales.data_ptr()),
      g_idx_ptr,
      reinterpret_cast<half*>(c.data_ptr()),
      reinterpret_cast<half*>(temp_dq.data_ptr()),
      size_m,
      size_n,
      size_k,
      groups,
...
void gptq_shuffle(
  ...
  int* perm_ptr = q_perm.size(0) == 0 ? nullptr
      : reinterpret_cast<int*>(q_perm.data_ptr());

  sglang::gptq::shuffle_exllama_weight(
      reinterpret_cast<uint32_t*>(q_weight.data_ptr()),
      perm_ptr,
      static_cast<int>(q_weight.size(0) * 32 / bit),
