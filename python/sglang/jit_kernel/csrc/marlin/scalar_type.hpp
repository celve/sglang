...
// Standalone: replaced <torch/library.h> with marlin_check.h
...
class ScalarType {
 public:
  ...
  static constexpr ScalarType float_IEEE754(uint8_t exponent, uint8_t mantissa) {
    MARLIN_CHECK(mantissa > 0 && exponent > 0);
  ...
  static constexpr ScalarType float_(uint8_t exponent, uint8_t mantissa, bool finite_values_only, NanRepr nan_repr) {
    MARLIN_CHECK(nan_repr < NAN_REPR_ID_MAX, "Invalid NanRepr");
    MARLIN_CHECK(mantissa > 0 && exponent > 0);
    MARLIN_CHECK(
 ...
 private:
  double _floating_point_max() const {
    MARLIN_CHECK(mantissa <= 52 && exponent <= 11, "Cannot represent max/min as a double for type ", str());
    ...
    if (nan_repr == NAN_EXTD_RANGE_MAX_MIN || nan_repr == NAN_NONE) {
      MARLIN_CHECK(exponent < 11, "Cannot represent max/min as a double for type ", str());
      max_exponent += 1;
    }
  ...
  constexpr std::variant<int64_t, double> _raw_max() const {
    if (is_floating_point()) {
      ...
      MARLIN_CHECK(size_bits() < 64 || size_bits() == 64 && is_signed(), "Cannot represent max as a int64_t");
  ...
  constexpr std::variant<int64_t, double> _raw_min() const {
    if (is_floating_point()) {
      MARLIN_CHECK(is_signed(), "We currently assume all floating point types are signed");
      ...
      MARLIN_CHECK(!is_signed() || size_bits() <= 64, "Cannot represent min as a int64_t");
      if (is_signed()) {
