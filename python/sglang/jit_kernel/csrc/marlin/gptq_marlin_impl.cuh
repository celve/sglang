 ...
 * Converted to header-only (.cuh) for JIT compilation.
 * - TORCH_CHECK -> MARLIN_CHECK
 * - gptq_marlin_gemm() entry point removed (replaced by TVM FFI wrapper)
 * - Preprocessor guards added for JIT weight-type selection
 */
...
MarlinFuncPtr get_marlin_kernel(
  ...
  // JIT preprocessor guards: when MARLIN_JIT is defined, only compile
  // the weight type(s) whose defines are set.
#if !defined(MARLIN_JIT) || defined(MARLIN_WTYPE_KU4)
  COMMON_GET_IF(sglang::kU4)
#endif

#if !defined(MARLIN_JIT) || defined(MARLIN_WTYPE_KU4B8)
  COMMON_GET_IF(sglang::kU4B8)
#endif

#if !defined(MARLIN_JIT) || defined(MARLIN_WTYPE_KU8B128)
  COMMON_GET_IF(sglang::kU8B128)
#endif

#if !defined(MARLIN_JIT) || defined(MARLIN_WTYPE_KFE2M1F)
  FP4_GET_IF(sglang::kFE2M1f)
#endif

#if !defined(MARLIN_JIT) || defined(MARLIN_WTYPE_KFE4M3FN)
...
void marlin_mm(
  ...
  MARLIN_CHECK(prob_m > 0 && prob_n > 0 && prob_k > 0, "Invalid MNK = [", prob_m, ", ", prob_n, ", ", prob_k, "]");
  ...
  while (rest_m) {
    ...
    if (thread_k != -1 && thread_n != -1) {
      ...
      MARLIN_CHECK(prob_n % thread_n == 0, "prob_n = ", prob_n, " is not divisible by thread_n = ", thread_n);
      MARLIN_CHECK(prob_k % thread_k == 0, "prob_k = ", prob_k, " is not divisible by thread_k = ", thread_k);
    } else {
      // Auto config
